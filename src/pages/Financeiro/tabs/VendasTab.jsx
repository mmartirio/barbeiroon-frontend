...
export default function VendasTab() {
  ...
  const buscarProduto = useCallback(async (term) => {
    ...
    try {
      ...
      if (d2.data?.length >= 1) setResultado(d2.data[0]);
  ...
  const removeCarrinho = (produtoId) => setCarrinho(c => c.filter(i => i.produto.id !== produtoId));

  const updateQtd = (produtoId, qtd) => {
    const n = parseInt(qtd, 10);
    if (isNaN(n) || n <= 0) return;
    setCarrinho(c => c.map(i => i.produto.id === produtoId ? { ...i, quantidade: n } : i));
  };
      ...
      <div className="card" style={{ padding: '1.25rem' }}>
        ...
        {sucesso && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(22,163,74,0.15)', border: '1px solid var(--success)', borderRadius: 6, padding: '0.6rem 1rem', marginBottom: '1rem', color: 'var(--success)', fontWeight: 700 }}>
            <FiCheckCircle size={16} /> Venda realizada com sucesso! Total: {sucesso}
          </div>
        )}
