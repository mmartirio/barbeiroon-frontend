...
function Badge({ status }) {
    ...
    return <span style={{ fontSize: '0.72rem', padding: '2px 8px', borderRadius: 4, background: s.bg, color: s.color, fontWeight: 600 }}>{STATUS_LABEL[status] || status}</span>;
}
...
function PixConfigTab() {
    ...
    const load = useCallback(async () => {
        try { const d = await api('/pix/config'); if (d) setForm({ keyType: d.keyType || 'cpf', keyValue: d.keyValue || '', ownerName: d.ownerName || '', city: d.city || '', bankName: d.bankName || '' }); }
    ...
    const handleSave = async (e) => {
        ...
        try { await api('/pix/config', { method: 'PUT', body: JSON.stringify(form) }); setSuccess('Configuração salva com sucesso!'); }
    ...
    return (
        <div style={{ maxWidth: 520 }}>
            <p style={{ color: 'var(--color-muted)', fontSize: '0.85rem', marginBottom: 20, lineHeight: 1.6 }}>
                Configure sua chave PIX. O sistema irá gerar um <strong>Pix Copia e Cola</strong> automaticamente em cada cobrança.
            </p>
            ...
            <form onSubmit={handleSave}>
                ...
                <div className="form-group">
                    ...
                    <input className="form-input" value={form.keyValue} onChange={e => setForm(p => ({ ...p, keyValue: e.target.value }))} required
                        placeholder={form.keyType === 'cpf' ? '000.000.000-00' : form.keyType === 'cnpj' ? '00.000.000/0000-00' : form.keyType === 'email' ? 'seu@email.com' : form.keyType === 'phone' ? '+55 11 99999-9999' : 'Chave aleatória (UUID)'} />
                </div>
                <div className="form-group">
                    ...
                    <input className="form-input" value={form.ownerName} onChange={e => setForm(p => ({ ...p, ownerName: e.target.value }))} required maxLength={25} placeholder="Ex: Barbeiroon Servicos" />
                    <span style={{ fontSize: '0.72rem', color: 'var(--color-muted)' }}>{form.ownerName.length}/25</span>
                </div>
                <div className="form-group">
                    <label className="form-label">Cidade * <span style={{ fontSize: '0.72rem', color: 'var(--color-muted)' }}>máx. 15 caracteres</span></label>
                    <input className="form-input" value={form.city} onChange={e => setForm(p => ({ ...p, city: e.target.value }))} required maxLength={15} placeholder="Ex: Sao Paulo" />
                </div>
                <div className="form-group">
                    <label className="form-label">Banco (opcional)</label>
                    <input className="form-input" value={form.bankName} onChange={e => setForm(p => ({ ...p, bankName: e.target.value }))} placeholder="Ex: Nubank, Itaú, BB" maxLength={100} />
                </div>
...
function CobrancasTab() {
    ...
    const handleMarkPaid = async (inv) => {
        ...
        try { await api(`/pix/invoices/${inv.id}/paid`, { method: 'PUT' }); loadInvoices(); }
        catch (e) { alert('Erro: ' + e.message); }
    };

    const handleCancel = async (inv) => {
        if (!window.confirm('Cancelar esta cobrança?')) return;
        try { await api(`/pix/invoices/${inv.id}`, { method: 'DELETE' }); loadInvoices(); }
        catch (e) { alert('Erro: ' + e.message); }
    };
    ...
    return (
        <div>
            ...
            {!hasPixConfig && <div style={infoStyle}>Configure sua chave PIX na aba <strong>Chave PIX</strong> antes de emitir cobranças.</div>}
            ...
            {!loading && invoices.length === 0 && <p style={{ color: 'var(--color-muted)', textAlign: 'center', padding: 40 }}>Nenhuma cobrança encontrada.</p>}
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                                ...
                                {canAction && (
                                    <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                                        <button className="btn" onClick={() => handleMarkPaid(inv)} title="Marcar como pago" style={{ padding: '6px 10px', color: '#4ade80' }}><FiCheckCircle size={13} /></button>
                                        <button className="btn" onClick={() => handleCancel(inv)} title="Cancelar" style={{ padding: '6px 10px', color: '#f87171' }}><FiXCircle size={13} /></button>
                                    </div>
                                )}
                            </div>
                            {inv.pixEmv && (
                                <div style={{ marginTop: 10, padding: '10px 12px', background: 'rgba(0,0,0,0.2)', borderRadius: 6 }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                                        ...
                                        <button onClick={() => handleCopy(inv.id, inv.pixEmv)} style={{ background: 'none', border: 'none', color: copied === inv.id ? '#4ade80' : '#60a5fa', fontSize: '0.72rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                                            <FiCopy size={11} /> {copied === inv.id ? 'Copiado!' : 'Copiar'}
                                        </button>
                                    </div>
                                    <code style={{ fontSize: '0.7rem', wordBreak: 'break-all', opacity: 0.75, display: 'block' }}>{inv.pixEmv.slice(0, 100)}…</code>
                                </div>
                            )}
                        </div>
                    );
                    ...
                    <form onSubmit={handleCreate}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
                            ...
                            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                                ...
                                <input className="form-input" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Ex: Plano mensal maio/2025" maxLength={40} />
                            </div>
                            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                                <label className="form-label">Observações internas</label>
                                <textarea className="form-input" value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} rows={2} placeholder="Notas internas" style={{ resize: 'vertical' }} />
                            </div>
                        </div>
...
export default function GestorBilling() {
    ...
    const tabStyle = (key) => ({
        ...
        cursor: 'pointer', fontSize: '0.88rem', transition: 'color 0.15s',
    });
